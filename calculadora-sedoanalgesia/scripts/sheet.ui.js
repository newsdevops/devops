...
(function initModuleTabs(){
  ...
  // ✅ API GLOBAL: sincroniza UI del módulo (sedo/suero/hipo)
  window.setModuleUI = (module) => {
    ...
    // ✅ Oculta subtabs de Sedo en Suero e Hipoglucemia
    if (tabsSedo) {
      tabsSedo.style.display = (module === 'suero' || module === 'hipo') ? 'none' : '';
    }
  };

  function setModule(module){
    const isSuero = module === 'suero';
    const isHipo  = module === 'hipo';

    // ✅ sincroniza tabs (incluye hipo)
    window.setModuleUI(module);

    if (isSuero){
      window.setMainView?.('sueroterapia');
      window.setSueroView?.('calculo');
      window.renderSueroterapia?.();
    } else if (isHipo){
      window.setMainView?.('hipoglucemia');
