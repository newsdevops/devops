...
// Render suero SOLO si está visible
  const secSuero = document.getElementById('section-sueroterapia');
  if (secSuero && !secSuero.hidden && typeof renderSueroterapia === 'function') {
    renderSueroterapia();
  }
...
buildDrugDropdown('antidotos', 'drugDD-antidotos');
	...
	/*Tabs de módulo (desktop): Sedoanalgesia <-> Sueroterapia */
(function initModuleTabs(){
  const moduleTabs = document.querySelector('.tabs--module');
  const tabsSedo = document.querySelector('.tabs--sedo');
  const secSuero = document.getElementById('section-sueroterapia');

  if (!moduleTabs || moduleTabs.dataset.bound === '1') return;
  moduleTabs.dataset.bound = '1';

function setModule(module){
  const isSuero = module === 'suero';

  // activar tabs visualmente (lo de siempre)...

// activar tabs de módulo visualmente + aria
moduleTabs.querySelectorAll('button[data-module]').forEach(b => {
  const active = b.dataset.module === module;
  b.classList.toggle('is-active', active);
  b.setAttribute('aria-selected', active ? 'true' : 'false');
});

  // Mostrar/ocultar tabs sedo nivel 2
  if (tabsSedo) tabsSedo.style.display = isSuero ? 'none' : '';

  // ✅ CAMBIO CLAVE: usa setMainView (drawer.js) para sincronizar activo
  if (isSuero) {
    window.setMainView?.('sueroterapia');   // ← esto actualizará el drawer
    window.setSueroView?.('calculo');
    window.renderSueroterapia?.();
  } else {
    // Vuelve a la vista sedo que quieras (por ejemplo, sedantes o checklist)
    window.setMainView?.('sedantes');       // o 'checklist' si prefieres
  }

  window.scrollTo({ top: 0, behavior: 'smooth' });
  
window.setModuleUI = (module) => {
  // module: 'sedo' | 'suero'
  moduleTabs?.querySelectorAll('button[data-module]').forEach(b => {
    const active = b.dataset.module === module;
    b.classList.toggle('is-active', active);
  ...
  // mostrar/ocultar subtabs sedo nivel 2
  if (tabsSedo) tabsSedo.style.display = (module === 'suero') ? 'none' : '';
};
  ...
  // Estado inicial: Sedoanalgesia
  setModule('sedo');
})();
	
	if(typeof initSueroUI === 'function') initSueroUI();
