...
// ================================
// Splash: máscara del destello con la URL REAL del logo
// ================================
// Splash mask desactivada: ahora el barrido se resuelve solo con CSS.

const splash = document.getElementById('splash');
const splashStart = performance.now();

function hideSplashWithMinTime(minMs = 6500){
  if (!splash) return;
...
window.addEventListener('load', () => hideSplashWithMinTime(6500), { once: true });
...
buildDrugDropdown('antidotos', 'drugDD-antidotos');
	...
	// ================================
// Bottom bar: medir altura real
// ================================
function updateBottomBarVar(){
  const bar = document.getElementById('bottomBar');
  if(!bar) return;
  const h = Math.ceil(bar.getBoundingClientRect().height);
  document.documentElement.style.setProperty('--bottom-bar-real-h', `${h}px`);
}

// inicial + en cambios de layout
updateBottomBarVar();
window.addEventListener('resize', () => requestAnimationFrame(updateBottomBarVar));
window.addEventListener('orientationchange', () => requestAnimationFrame(updateBottomBarVar));

// si hay ResizeObserver, aún mejor
if ('ResizeObserver' in window) {
  const bar = document.getElementById('bottomBar');
  if(bar){
    new ResizeObserver(() => requestAnimationFrame(updateBottomBarVar)).observe(bar);
  }
}
