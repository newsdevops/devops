...
// --- AJUSTE: "MESSETA" de foco ---
// Mientras dist <= deadZone, target=1 (foco máximo "aguanta")
const deadZone = 140; // px (sube a 180-220 para que aguante más)
let target;

if (dist <= deadZone) {
  target = 1;
} else {
  // fuera de la meseta, bajada suave hasta 0 en 'range'
  const t = (dist - deadZone) / (range - deadZone);
  target = clamp01(1 - t);
}
