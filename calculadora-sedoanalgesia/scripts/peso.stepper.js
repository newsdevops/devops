...
// Slider -> PREVIEW (sin render pesado)
pesoRange.addEventListener("input", () => {
  if (isLocked()) return;
  // Actualiza valor visible y posición, pero NO dispara render
  setPesoValue(pesoRange.value, false);
});

// Slider -> COMMIT (render pesado solo al soltar)
const commitFromSlider = () => {
  if (isLocked()) return;
  setPesoValue(pesoRange.value, true); // aquí sí emit y render
};
