...
// sheet.js v20251222-1 — filas garantizadas, sticky sin solapes (tbody-offset = header + thead), caps/min Excel, trunc 3 dec sin ceros
(function(){
  'use strict';
...
function ensureTransposeStyles(){
  ...
  st.textContent = `
    ...
    /* Sticky horizontal: columna de etiquetas fija al hacer scroll */
    ...
    .tgrid__label{ font-weight:700; color:#374151; background:#f8fafc;
    ...
    .tgrid__cell.via-iv{ color:var(--via-iv,#C00000); font-weight:700; }
    .tgrid__cell.via-in{ color:var(--via-in,#3072C2); font-weight:700; }
    .tgrid__cell.via-im{ color:var(--via-im,#6CA62C); font-weight:700; }

    /* Modo estrecho: ocultar tabla y mostrar tarjetas */
    @media ${MQ_NARROW}{
      /* Header fijo (Peso) sin tapar el menú */
      .app-header{ position: fixed; top: 0; left: 0; right: 0; z-index: 300; }
      body{ padding-top: var(--app-header-h, 72px); }

      /* Asegurar que el menú/hamburguesa queda por encima del header fijo */
      .menu-backdrop{ z-index: 1200; }
      .side-menu{ z-index: 1300; }
