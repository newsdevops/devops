...
// sheet.js v20251215-4 — filas garantizadas, sticky sin solapes (tbody-offset = header + thead), caps/min Excel, trunc 3 dec sin ceros
(function(){
  'use strict';
...
// - No requiere cambios en HTML/CSS: se inyecta CSS y se crean contenedores si faltan.
// =========================================================
...
function ensureTransposeStyles(){
  ...
  st.textContent = `
    ...
    .drug-card{ background:#fff; border:1px solid var(--border,#e5e7eb); border-radius:1rem;
      box-shadow:0 8px 22px rgba(0,0,0,.06); overflow:hidden; margin-bottom:.9rem; }
    .drug-card__title{ font-weight:900; padding:.65rem .85rem; border-bottom:1px solid var(--border,#e5e7eb);
      text-transform:uppercase; letter-spacing:.02em; }
    .tgrid{ --cols:1; display:grid; grid-template-columns: 135px repeat(var(--cols), minmax(105px, 1fr)); }
    .tgrid__cell{ padding:.55rem .65rem; border-top:1px solid var(--border,#e5e7eb); }
    .tgrid__label{ font-weight:800; color:#374151; background:#f8fafc; }
    .tgrid__colhead{ font-weight:900; text-transform:uppercase; background:#fff; }
    .tgrid__cell.dose{ background:#9FC5E8; }
    .tgrid__cell.vol{ background:#9BBB59; }
    .tgrid__cell.limits{ background:#DD7E6B; text-align:center; white-space:nowrap; }
...
function render(){
  ...
  // Vista traspuesta en modo estrecho (sin scroll horizontal)
  if(isNarrow()){
    ensureTransposeStyles();
    // Oculta tablas y pinta tarjetas
    ...
    // Modo ancho: mostrar tablas y ocultar tarjetas
  ...
  // Mostrar columna fármaco SOLO cuando filtro = null (TODOS)
